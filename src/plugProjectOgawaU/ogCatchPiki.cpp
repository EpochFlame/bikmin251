...
void CallBack_CatchPiki::init(J2DScreen* screen, u64 tag, u32* pikiType, JKRArchive* archive)
{
	...
	for (int i = 0; i < ARRAY_SIZE(PikiIconTextureName); i++) {
...
void CallBack_CatchPiki::update()
{
	if (m_pikiType && m_pikiIcon) {
		...
		if (pikiType != m_currPikiType && pikiType < ARRAY_SIZE(PikiIconTextureResTIMG)) {
			setPikiIcon(pikiType);
		...
		m_pikiIcon->rotate(m_pikiIcon->m_bound.getWidth() / 2, m_pikiIcon->m_bound.getHeight() / 2, J2DROTATE_Z, 0.0f);
...
{
	if (pikiType < ARRAY_SIZE(PikiIconTextureResTIMG)) {
