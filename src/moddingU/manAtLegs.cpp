...
void Obj::startMatAnimator() { sMatAnimator.start(C_MGR->mTevRegAnimation); }
...
void Obj::setShotGunTargetPosition()
{
	...
	CI_LOOP(iter)
	{
		...
		if (baby->m_FSM->getCurrID(baby) != Baby::BABY_Dead && baby->isAlive()) {
			mShotGunTargetPosition = baby->getPosition();
		}
	}
	...
	if (mShotGunBurstTimer > 1.0f) {
		...
		mShotGunTargetPosition
		    = Vector3f(randDist * sinf(randAngle) + m_position.x, m_position.y, randDist * cosf(randAngle) + m_position.z);
	}
